...
class MyWindow(QWidget):
    ...
    def init_ui(self):
        self.ui = uic.loadUi("./mpu6050.ui")
        self.X_angle = self.ui.textBrowser
        self.servo_angle = self.ui.textBrowser_2
        self.Y_angle = self.ui.textBrowser_3
        self.Z_angle = self.ui.textBrowser_4
        ...
        if self.ser.is_open:
            ...
            if self.rc_data != b'':
                ...
                if data.find('X') != -1:
                    ...
                    Z = data.find('Z')
                    S = data.find('S')
                    self.X_angle.setPlainText(data[X + 1: Y])
                    self.Y_angle.setPlainText(data[Y + 1: Z])
                    self.Z_angle.setPlainText(data[Z + 1: S])
                    self.servo_angle.setPlainText(data[S + 1:])
